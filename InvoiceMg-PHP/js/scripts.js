...
$(document).ready(function() {
	...
	var dateFormat = $(this).attr('data-vat-rates');
	$('#invoice_date, #invoice_due_date').datetimepicker({
		showClose: false,
		format: dateFormat
	});
	...
	function calculateTotal() {
		...
		var subT = parseFloat(grandTotal),
	    	finalTotal = parseFloat(grandTotal + c_ship),
	    	vat = parseInt($('.invoice-vats').attr('data-vat-rates'));
		...
		if($('.invoice-vats').attr('data-enable-vats') === '1') {

	        if($('.invoice-vats').attr('data-vat-methods') === '1') {
		        $('.invoice-vats').text(((vat / 100) * finalTotal).toFixed(2));
		        $('#invoice_vats').val(((vat / 100) * finalTotal).toFixed(2));
	            $('.invoice-total').text((finalTotal).toFixed(2));
	            $('#invoice_total').val((finalTotal).toFixed(2));
	        } else {
	            $('.invoice-vats').text(((vat / 100) * finalTotal).toFixed(2));
		...
		if($('input.remove_vats').is(':checked')) {
	        $('.invoice-vats').text("0.00");
	        $('#invoice_vats').val("0.00");
            $('.invoice-total').text((finalTotal).toFixed(2));
            $('#invoice_total').val((finalTotal).toFixed(2));
	    }
